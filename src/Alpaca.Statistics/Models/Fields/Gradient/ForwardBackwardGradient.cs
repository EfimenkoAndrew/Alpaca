        ...
        IHiddenRandomFieldGradient, IDisposable
    {
        ...
        public double[] Gradient()
        {
            ...
            for (int i = 0; i < inputs.Length; i++)
            {
                ...
                Alpaca.Clustering.Core.Debug.Assert(!Double.IsNaN(error));
            }
        ...
        private void InnerGradient(FactorPotential<T> factor, T[][] inputs, int[] outputs, double[] lnZx, double[] lnZxy, double[] gradient)
        {
            ...
            for (int k = factor.FactorParameters.Offset; k < end; k++)
            {
                ...
                for (int i = 0; i < inputs.Length; i++)
                {
                    ...
                    Alpaca.Clustering.Core.Debug.Assert(!marginals.HasNaN());
                    Alpaca.Clustering.Core.Debug.Assert(!Double.IsNaN(lnsum1));
                    Alpaca.Clustering.Core.Debug.Assert(!Double.IsNaN(lnsum2));
                }
                ...
                Alpaca.Clustering.Core.Debug.Assert(!Double.IsNaN(derivative));
