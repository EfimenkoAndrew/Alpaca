...
namespace Alpaca.Evaluation.Internal
{
...

    {
        ...
        public double Evaluate(ClusterSet<TInstance> clusterSet)
        {
            ...
            return clusterSet.Count < 2 ? double.NaN : EvaluateEach(clusterSet).Values.Average();
        }
            ...
            for (var i = 0; i < clusterSet.Count; i++)
            {
                ...
                for (var j = 0; j < cluster.Count; j++)
                {
                    ...
                    for (var k = 0; k < cluster.Count; k++)
                        if (k != j)
                            avgWithinDist += DissimilarityMetric.Calculate(instance, cluster[k]);
                    ...
                    for (var l = 0; l < clusterSet.Count; l++)
                    {
                        ...
                        var avgBetweenDist = clusterSet[l]
                            .Average(other => DissimilarityMetric.Calculate(instance, other));
