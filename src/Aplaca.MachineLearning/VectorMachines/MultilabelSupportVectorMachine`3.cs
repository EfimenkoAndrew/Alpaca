...
namespace Accord.MachineLearning.VectorMachines
{
...
#endif
    {
        ...
        private double distance(int index, TInput input, Cache cache)
        {
            ...
            Alpaca.Clustering.Core.Debug.Assert(sum.IsEqual(expected, rtol: 1e-3));
#endif
        ...
        public override double[] LogLikelihoods(TInput input, ref bool[] decision, double[] result)
        {
            ...
            for (int i = 0; i < result.Length; i++)
            {
                ...
                Alpaca.Clustering.Core.Debug.Assert(result[i].IsEqual(expectedScore, rtol: 1e-3));
#endif
                result[i] = -Special.Log1pexp(-result[i]);
#if DEBUG
                double expectedLogLikelihood = this.Models[i].LogLikelihood(input);
                Alpaca.Clustering.Core.Debug.Assert(result[i].IsEqual(expectedLogLikelihood, rtol: 1e-3));
#endif
