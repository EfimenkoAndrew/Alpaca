...
using Alpaca.Math.Convergence;
using Alpaca.Math.Convergence.Base;
            ...
            do
            {
...
#endif
                {
                    for (int j = 0; j < states; j++)
                    {
                        ...
                        Alpaca.Clustering.Core.Debug.Assert(!Double.IsNaN(logA[i][j]));
                    }
                }
#if !SERIAL
                );
        ...
        private double Expectation(TObservation[][] x, int maxLength)
        {
            ...
            Parallel.For(0, x.Length, ParallelOptions,
...
#endif
                {
                    ...
                    Alpaca.Clustering.Core.Debug.Assert(!Double.IsNaN(lnsum));
