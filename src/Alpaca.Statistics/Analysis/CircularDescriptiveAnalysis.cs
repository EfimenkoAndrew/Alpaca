...
#pragma warning restore 612, 618
    {
            ...
            get
            {
                if (innerFences == null)
                {
                    ...
                    for (int i = 0; i < innerFences.Length; i++)
                    {
                        ...
                        if (useStrictRanges)
                        {
                            innerFences[i].Min = Alpaca.Math.Tools.Mod(innerFences[i].Min, lengths[i]);
                            innerFences[i].Max = Alpaca.Math.Tools.Mod(innerFences[i].Max, lengths[i]);
                        }
                    }
                }
            ...
            get
            {
                if (outerFences == null)
                {
                    ...
                    for (int i = 0; i < outerFences.Length; i++)
                    {
                        ...
                        if (useStrictRanges)
                        {
                            outerFences[i].Min = Alpaca.Math.Tools.Mod(outerFences[i].Min, lengths[i]);
                            outerFences[i].Max = Alpaca.Math.Tools.Mod(outerFences[i].Max, lengths[i]);
                        }
                    }
                }
        ...
        public DoubleRange GetConfidenceInterval(int index, double percent = 0.95)
        {
            ...
            if (useStrictRanges)
            {
                min = Alpaca.Math.Tools.Mod(min, lengths[index]);
                max = Alpaca.Math.Tools.Mod(max, lengths[index]);
            }
